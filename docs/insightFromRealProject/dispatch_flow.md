...
## Monthly Driver Reports (out-of-flow, manual feature)

`GET /api/reports/monthly/preview` → `POST /api/reports/monthly/send`

The dispatcher manually generates and sends per-driver Hebrew PDF summaries via WhatsApp at month end. After sending, driver replies are forwarded to Michel for 7 days (`REPORT_FORWARD_WINDOW_DAYS`).

> **⚠️ Decision pending (2026-05-07):** Operations team has requested that billing and reporting remain in Ride Control. RC dev team is tasked with building per-driver reports on their platform. **This feature stays in our codebase unchanged until a final decision is made.** See `docs/ideas.md` for full context.
